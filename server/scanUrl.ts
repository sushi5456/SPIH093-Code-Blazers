...
import type { ScanUrlResponse } from "./types.js";
...
async function vtFetch(path: string, apiKey: string, init?: RequestInit): Promise<globalThis.Response> {
...
async function pollAnalysis(
  analysisId: string,
  apiKey: string
): Promise<VTAnalysisAttributes | null> {
  const deadline = Date.now() + POLL_TIMEOUT_MS;
  while (Date.now() < deadline) {
    const response = await vtFetch(`/analyses/${analysisId}`, apiKey, { method: "GET" });
    if (response.status === 429) throw new Error("VirusTotal rate limit reached. Try again shortly.");
    if (!response.ok) throw new Error(`VirusTotal analysis lookup failed (${response.status}).`);

    const body = (await response.json()) as VTAnalysisResponse;
...
export async function scanUrlHandler(req: Request, res: Response): Promise<void> {
  try {
    ...
    const submitBody = (await submitResponse.json()) as VTSubmitResponse;
    ...
    if (!attributes) {
      const timeoutResponse: ScanUrlResponse = {
        url: normalized,
        score: 0,
        verdict: "UNKNOWN",
    ...
    let verdict: ScanUrlResponse["verdict"];
    ...
    const response: ScanUrlResponse = {
      url: normalized,
      score,
      verdict,
...
export function healthHandler(_req: Request, res: Response): void {
  const apiKeyConfigured = Boolean(
    process.env.VIRUSTOTAL_API_KEY && process.env.VIRUSTOTAL_API_KEY !== "your_actual_key_here"
  );
